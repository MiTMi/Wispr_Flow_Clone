...
Instead of trying to fit the window _exactly_ to the screen (which is prone to OS interference), we intentionally make the window **larger than the screen** and position it at **negative coordinates**.
...
- **Overcomes OS Constraints:** By requesting a negative X coordinate (`-100`), we force the window to extend past the left edge. Even if macOS clamps it slightly, the extra width ensures the _visible_ screen area is fully covered by the window.
...
- `left: 50%` places the element at the exact center of the _window_.
