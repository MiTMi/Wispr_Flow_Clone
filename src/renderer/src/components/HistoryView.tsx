...
function HistoryView(): React.JSX.Element {
    ...
    const renderHistoryItem = (item: HistoryItem, index: number, groupLength: number) => (
        <div key={item.id} className={`bg-white p-4 flex gap-6 hover:bg-zinc-50 transition-colors group relative ${index !== groupLength - 1 ? 'border-b border-zinc-100' : ''}`}>
            <span className="text-xs font-medium text-zinc-400 w-16 pt-1">{formatTime(item.timestamp)}</span>
            <p className="text-zinc-800 text-sm leading-relaxed flex-1 pr-20">{item.text}</p>

            {/* Actions */}
            <div className={`absolute right-4 top-4 flex items-center gap-1 transition-opacity ${openMenuId === item.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                <button
                ...
                <div className="relative">
                    ...
                    {openMenuId === item.id && (
                        <div ref={menuRef} className="absolute right-0 top-8 bg-white shadow-xl border border-zinc-200 rounded-lg w-48 z-20 py-1 flex flex-col animate-in fade-in zoom-in-95 duration-100">
            ...
            <div className="max-w-4xl mx-auto p-8">
                ...
                <div className="bg-[#FEFCE8] border border-yellow-100 rounded-2xl p-8 mb-12 relative overflow-hidden">
                    <div className="relative z-10">
                        ...
                        <p className="text-zinc-600 max-w-lg mb-6 text-sm leading-relaxed">
                            Flow adapts to how you write in different apps. Personalize your style for <span className="font-semibold text-zinc-900">messages, work chats, emails, and other apps</span> so every word sounds like you.
                        </p>
                ...
                <div className="space-y-8 pb-20">
                    {groupedHistory['TODAY'].length > 0 && (
                        <div>
                            <h3 className="text-xs font-semibold text-zinc-400 tracking-wider mb-4 uppercase">Today</h3>
                            <div className="space-y-0 border border-zinc-200 rounded-xl overflow-visible">
                                {groupedHistory['TODAY'].map((item, index) => renderHistoryItem(item, index, groupedHistory['TODAY'].length))}
                            </div>
                        </div>
                    )}
                    ...
                    {groupedHistory['YESTERDAY'].length > 0 && (
                        <div>
                            <h3 className="text-xs font-semibold text-zinc-400 tracking-wider mb-4 uppercase">Yesterday</h3>
                            <div className="space-y-0 border border-zinc-200 rounded-xl overflow-visible">
                                {groupedHistory['YESTERDAY'].map((item, index) => renderHistoryItem(item, index, groupedHistory['YESTERDAY'].length))}
                            </div>
                        </div>
                    )}
                </div>
