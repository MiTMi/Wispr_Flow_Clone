...
function SettingsView(): React.JSX.Element {
    ...
    useEffect(() => {
        ...
        const handleRecorded = (_: any, key: string) => {
            ...
            window.electron.ipcRenderer.send('update-setting', 'hotkey', key)
        }

        const handleHoldRecorded = (_: any, keycode: number) => {
            setHoldKey(keycode)
            setIsRecordingHoldKey(false)
            window.electron.ipcRenderer.send('update-setting', 'holdKey', keycode)
        }
            ...
            <div className="max-w-2xl mx-auto p-10">
                ...
                <div className="space-y-10">
                    ...
                    <section className="space-y-6">
                        <h2 className="text-lg font-semibold text-zinc-900 border-b border-zinc-100 pb-2">General</h2>
                        <div className="flex items-center justify-between">
                            <div>
                                <div className="font-medium text-zinc-900">Start on Login</div>
                                <div className="text-sm text-zinc-500">Automatically start Flow when you log in.</div>
                            </div>
                            <label className="relative inline-flex items-center cursor-pointer">
                                <input
                                    type="checkbox"
                                    checked={startOnLogin}
                                    onChange={(e) => { setStartOnLogin(e.target.checked); updateSetting('startOnLogin', e.target.checked); }}
                    ...
                    {/* Shortcuts */}
                    <section className="space-y-6">
                        <h2 className="text-lg font-semibold text-zinc-900 border-b border-zinc-100 pb-2">Shortcuts</h2>
                        ...
                        {triggerMode === 'toggle' ? (
                            ...
                            <div className="space-y-2">
                                ...
                                >
                                    {isRecordingHoldKey ? 'Press any key...' : (holdKey ? `Key Code: ${holdKey}` : 'Click to set key')}
                                </button>
